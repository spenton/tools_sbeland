...
;
; REVISION HISTORY:
;   Revision: $Id: align_test2.pro,v 1.1 2018/11/26 16:13:48 spenton Exp $
;-
;*******************************************************************
function my_amoeba_test2, coeffs
    common myamoeba2_common, sp, rsp, no_deriv, wave_range, use_rms, imode, dettempcorr
    ...
    if n_elements(coeffs) eq 1 then begin
        ...
        newx = ccd2lambda(iMode, sp.x, sp.prismtemp, gamz=gamz[0])
    endif else if n_elements(coeffs) eq 2 then begin
        gamz=(1d + coeffs[0])*sim_model.gamz[pos]
        pix =(1d + coeffs[1])*sim_model.pix[pos]
        newx = ccd2lambda(iMode, sp.x, sp.prismtemp, gamz=gamz[0], pixsize=pix[0])
    endif else if n_elements(coeffs) eq 3 then begin
        gamz=(1d + coeffs[0])*sim_model.gamz[pos]
        yab =(1d + coeffs[1])*sim_model.yab[pos]
        cz =(1d + coeffs[2])*sim_model.cz[pos]
        newx = ccd2lambda(iMode, sp.x, sp.prismtemp, gamz=gamz[0], yab=yab[0], cz=cz[0])
    endif else if n_elements(coeffs) eq 8 then begin
        gamz=(1d + coeffs[0])*sim_model.gamz[pos]
        pix =(1d + coeffs[1])*sim_model.pix[pos]
        wedge =(1d + coeffs[2])*sim_model.thp[pos]
        foc =(1d + coeffs[3])*sim_model.foc[pos]
        cz =(1d + coeffs[4])*sim_model.cz[pos]
        fref =(1d + coeffs[5])*sim_model.fref[pos]
        yab =(1d + coeffs[6])*sim_model.yab[pos]
        ;tref =(1d + coeffs[7])*sim_model.TREF_SILICA
        tref =sim_model.TREF_SILICA
        newx = ccd2lambda(iMode, sp.x, sp.prismtemp, $
            gamz=gamz[0], pix=pix[0], wedge=wedge[0], foc=foc[0], cz=cz[0], fref=fref[0], yab=yab[0],tref=tref[0])
    endif
    ...
    if use_rms eq 1 then begin
        ...
        corr_value = abs(1d - abs(correlate(newy[pos], rsp.y[pos], /double))) * 1d20
    endelse
...
function align_test2, startTime, stopTime, instrumentModeId, $
         ...
         detCorr=detCorr, order=order, solardist=solardist, no1au=no1au
    ...
    if size(spect,/tname) ne 'STRUCT' then begin
        ...
        if instrumentModeId gt 32 then begin
            spect = get_sim_spectra(startTime, stopTime, instrumentModeId, gps=gps, missionDays=missionDays, $
                julianDays=julianDays, version=version, /noCcdCorr, /noTempCorr, no1au=no1au, $
                solarDist=solarDist,profile_data=profile_data, /uncorr)
        endif else begin
    ...
    if n_elements(suba) lt 20 then begin
        ...
        newRefSpect.DN_TEMPCORR = newRefSpect.DN
    ...
    init_coef=replicate(0d, 8)
    ;  gamz=gamz, pixsize=pixsize, wedge=wedge, focal=focal, cz=cz, fref=fref, yab=yab, tref=tref
    coeffs = AMOEBA(1d-8, P0=init_coef, scale=1d-6, FUNCTION_VALUE=fval, FUNCTION_NAME='my_amoeba_test2', nmax=maxiter)
    ...
    coeffs[2] =(1d + coeffs[2])*sim_model.thp[iMode_pos]
    coeffs[3] =(1d + coeffs[3])*sim_model.foc[iMode_pos]
    coeffs[4] =(1d + coeffs[4])*sim_model.cz[iMode_pos]
    coeffs[5] =(1d + coeffs[5])*sim_model.fref[iMode_pos]
    coeffs[6] =(1d + coeffs[6])*sim_model.yab[iMode_pos]
    ;coeffs[7] =(1d + coeffs[6])*sim_model.TREF_SILICA
    coeffs[7] =sim_model.TREF_SILICA

    outspect=spect
    outspect.wavelength = ccd2lambda(iMode, outspect.ccdpos_uncorr, outspect.prismtemp, $
        gamz=coeffs[0], pix=coeffs[1], wedge=coeffs[2], foc=coeffs[3], cz=coeffs[4], $
        fref=coeffs[5], yab=coeffs[6], tref=coeffs[7])
    ...
    ref_ccdpos = lambda2ccd(iMode, refspect.wavelength, spect.prismTemp)
    new_ccdpos = lambda2ccd(iMode, refspect.wavelength, spect.prismTemp, $
        gamz=coeffs[0], pix=coeffs[1], wedge=coeffs[2], foc=coeffs[3], cz=coeffs[4], fref=coeffs[5], yab=coeffs[6], tref=coeffs[7])
    ...
    if NOT keyword_set(no_plot) then begin
        plot_multi,refSpect.ccdpos_uncorr, delta_ccdpos, outspect.ccdpos_uncorr, poly(outspect.ccdpos_uncorr, ccdfit), $

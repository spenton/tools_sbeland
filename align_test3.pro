...
;
; REVISION HISTORY:
;   Revision: $Id: align_test3.pro,v 1.1 2018/11/26 16:13:48 spenton Exp $
;------------------------------------------------------------------
;
...
function align_test3, startTime, stopTime, instrumentModeId, $
    ...
    minw=max([min(refSpect.wavelength>0.0),min(Spect.wavelength>0.0)])
    maxw=min([max(refSpect.wavelength),max(Spect.wavelength)])
    suba=where(refSpect.wavelength ge minw and refSpect.wavelength le maxw)
    subb=where(Spect.wavelength ge minw and Spect.wavelength le maxw)
    refWave = refSpect[suba].wavelength

    newSpect={wavelength:spect[subb].wavelength, dn:spect[subb].dn, prismTemp:spect[subb].prismTemp}
    ...
    if n_elements(wrange) eq 2 then begin
        ...
        rsp={x:refWave[wpos], y:(refSpect[suba].dn)[wpos]}
        sp={x:newSpect.wavelength[wpos], y:newSpect.dn[wpos]}
    endif else begin
        rsp={x:refWave, y:refSpect[suba].dn}
        sp={x:newSpect.wavelength, y:newSpect.dn}
    endelse
    ...
    new_x = poly(spect.wavelength,coeffs)
    outspect=spect
    outspect.wavelength = new_x

    ; this new wavelength is at the actual prismTemp -> get the corresponding ccdpos at refTemp
    dndt = interpol(profile_data.y18, profile_data.y4, outspect.wavelength, /spline)
    refTemp_wavelength = outspect.wavelength - (outspect.prismtemp - profile_data[0].y0) * dndt

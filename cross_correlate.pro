; $Id: cross_correlate.pro,v 1.1 2018/11/26 16:13:48 spenton Exp $
;+
;*NAME:
;			cross_correlate
;
...
if n_params(0) lt 1 then $
  message,'cross_correlate,s1,s2,offset,corr,ishift=ishift,width=width,i1=i1,i2=i2'
	...
	approx = round(ishift) ;nearest integer
...
;
	...
	it2_end   = (i2-approx-width2) < (n_elements(s2)-1)
...
;
		it1_start = (it2_start - width2 + approx + i) > 0
		it1_end = (it1_start + nt -1) < (n_elements(s1)-1)
...
;
	maxc = max(corr,k)
	if (k eq 0) or (k eq (width-1)) then begin

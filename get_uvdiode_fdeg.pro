...
;
; REVISION HISTORY:
;   Revision: $Id: get_uvdiode_fdeg.pro,v 1.1 2018/11/26 16:13:48 spenton Exp $
;------------------------------------------------------------------
function get_uvdiode_fdeg, starttime, stoptime, wavelength, version=version, bin55=bin55, step55=step55, $
    ...
    if size(inspectrum,/tname) ne 'STRUCT' then begin
        ...
        for pl=0, nspect-1L do begin
            ...
            s=sort(resA.wavelength)
            resA= resA[s]
            s=sort(resB.wavelength)
            resB= resB[s]
    ...
    for j=0L,nspect-1L do begin
        irradA[j]=interpol((*inspectrum.simA.diode[j]).irradiance,(*inspectrum.simA.diode[j]).wavelength,wavelength,/spline)
        val=min(abs((*inspectrum.simA.diode[j]).wavelength - wavelength),pos)
        ...
        irradB[j]=interpol((*inspectrum.simB.diode[j]).irradiance,(*inspectrum.simB.diode[j]).wavelength,wavelength,/spline)
        val=min(abs((*inspectrum.simB.diode[j]).wavelength - wavelength),pos)
    ...
    if not keyword_set(noplot) then begin
        ...
        lineplot,time_f[keep0],f_factor[keep0]/kappadeg,psym=-3,ptitle='F_Factor', xtitle='Mission Day',ytitle='F/Kappa',$

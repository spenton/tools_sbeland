 function interp2d, A, x0, y0, x1, y1, nxny,nearest_neighbor=nearest_neighbor,$
        extrapolate=extrapolate,trigrid=trigrid,_extra=extra, quiet=quiet
;+
; NAME:
;   interp2d
; PURPOSE:
...
if n_elements(x0) ne n_elements(a[*,0]) or n_elements(y0) ne n_elements(a[0,*]) then begin
...
if keyword_set(trigrid) then begin
  ...
  if ~keyword_set(quiet) then print,'% nx,ny: ',nxny[0],nxny[1]
...
;   y0 = transpose(reform(temporary(y0),nx,ny))
     ...
     y0 = transpose(rebin(temporary(y0), ny, nx, /sample))
  ENDIF 
...
if ~keyword_set(quiet) then print, '% nearest_neighbor:', (n_elements(nearest_neighbor) gt 0) ? nearest_neighbor:0

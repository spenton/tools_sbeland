...
;
; CALLING SEQUENCE:
;   PLOT_TSIS_TELEM, starttime, stoptime, /mission, /sima
;                      
; INPUT KEYWORDS:
;   startTime -
...
; REVISION HISTORY:
;   2013.12.22  SBeland
;   Revision: $Id: plot_tsis_telem.pro,v 1.1 2018/11/26 16:13:48 spenton Exp $
;-
;*****************************************************************************
pro GEN_PLOT_TSIS_TELEM, xval, yval, xrange=xrange, colors=colors, labels=labels, psfile=psfile, resolution=resolution
...
END
;-----------------------------------------------------------------------------
PRO PLOT_TSIS_TELEM,  startTime, stopTime, psfile=psfile, $
         gps=gps, missionDays=missionDays, julianDays=julianDays, vms=vms, $
         outdata=outdata, extelem=extelem, items=items, tlmid=tlmid, $
         clean=clean, silent=silent, resolution=resolution, missionname=missionname


    if not keyword_set(silent) then $
        print, 'PLOT_TSIS_TELEM ('+systime()+'):  Processing time range: ',startTime,' to ',stopTime

    if n_elements(missionname) eq 0 then missionname='TSIS'
    ...
    if size(outdata,/tname) ne 'STRUCT' then begin
        if n_elements(items) eq 0 and n_elements(tlmid) eq 0 then begin
            item=['relay_cone','cone_a_state','cone_b_state','cone_c_state','cone_d_state','dsp_eclipse_md']
        endif 
        ; get the data
        get_tsis_telemetry,outdata,info,t0, t1, /gps, sid=4, itemName=items, tlmid=tlmid, $
            externalElement=extelem, missionname=missionname
    endif
    items = tag_names(outdata)
    xval=ptrarr(n_elements(items))
    yval=ptrarr(n_elements(items))

    ; remove the outliers before plotting the data if requested
    ...
    gen_plot_tsis_telem,xval,yval, labels=items, psfile=psfile, xrange=gps2jd([t0,t1]/1d6), resolution=resolution

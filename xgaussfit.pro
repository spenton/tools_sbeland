...
pro xgaussfit_event,event
	...
	'DONE': begin
      ...
      tvlct,base.rsave,base.gsave,base.bsave
		x=0
		y=0
		yfit=0
		base=0
	...
	'PLOT1': begin
		...
		if base.state eq '' then begin
			...
			plots,[xd,xd],[0,200],/dev,color=2
			wset,base.plot1_id
			device,copy=[0,0,700,400,0,0,base.pixmap1]
			plots,[xd,xd],[0,400],/dev,color=2
			plots,[0,700],[yd,yd],/dev,color=2
		end
		end
	'PLOT2': begin
		xd = event.x
...
;
	...
	plot,x,y,xrange=[xmin,xmax],xstyle=1,yrange=[ymin1,ymax1],ystyle=1, $
		xtitle=base.xtitle,ytitle=base.ytitle,title='',/nodata,color=0
;
	if ngauss gt 0 then begin
	    for i=0,ngauss-1 do begin
		pos = npoly+i*3
		gaussx,xx,coef(pos+1),coef(pos+2),coef(pos),g
		oplot,xx,g+ybase,color=8
		oplot,coef(pos+1)+fltarr(2),!y.crange,color=8
		xyouts,coef(pos+1), $
			!y.crange(1)+0.01*(!y.crange(1)-!y.crange(0)), /data, $
			strtrim(i+1,2),align=0.5,size=csize,color=0
	    end
	end
	oplot,xx,ybase,color=2
	oplot,x,y,color=0
	oplot,x,y,color=9,psym=4,symsize=symsize
	good = where(mask,ngood)
	if ngood gt 0 then oplot,x(good),y(good),color=0,psym=4,symsize=symsize
	oplot,xx,yyfit,color=2,thick=2
...
;
	if keyword_set(ps) then begin
		xyouts,0.50,0.95,base.title,/norm,color=0,align=0.5,size= 1.3
		if npoly gt 0 then begin
			xyouts,0.1,0.2,'Baseline Coefficients',color=0,/norm
			for i=0,npoly-1 do xyouts,0.15,0.17-i*0.02,/norm, $
						color=0,strtrim(coef(i),2)
		end
		if ngauss gt 0 then begin
		    xyouts,0.45,0.2,/norm,color=0,  $
			'           Peak             Center           FWHM'
		    for i=0,ngauss-1 do xyouts,0.45,0.17-i*0.02, $
				/norm,color=0,strtrim(i+1,2)+ $
...
;
	...
	base = {plot1:plot1,xmin:xmin,xmax:xmax,ymin1:ymin1,ymin2:ymin2, $
		...
		psave1:!p, psave2:!p, rsave:rsave, gsave:gsave, bsave:bsave}
;
; perform intial fit
;
